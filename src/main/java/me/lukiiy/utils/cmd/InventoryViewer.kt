...
object InventoryViewer : Listener {
    private val permission = "lukitils.invsee"
    ...
    private fun handle(sender: Player, target: Player, act: (Player, Player) -> Unit, actDesc: String) {
        sender.sendMessage(Defaults.msg(Component.text("Inspecting ").append(target.name().color(Defaults.YELLOW)).append(Component.text("'s $actDesc"))))
        act(sender, target)
    }
}